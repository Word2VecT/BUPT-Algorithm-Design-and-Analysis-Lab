...
{
    // 构造文件名
    std::string input_filename = "/Users/tang/Course/算法设计与分析/Lab/Lab 1/random_numbers/random_numbers_" + std::to_string(nDefaultCount) + ".txt";
    ...
    // 构造输出文件名
    std::string output_filename = "/Users/tang/Course/算法设计与分析/Lab/Lab 1/times/determine_pro_" + std::to_string(nDefaultCount) + ".txt";
    std::ofstream outfile(output_filename, std::ios::app);
    if (!outfile) {
        std::cerr << "无法以追加模式打开文件: " << output_filename << '\n';
        return 1;
    }
    ...
    for (int i = 1; i <= kTimes; ++i) {
        ...
        outfile << std::fixed << std::setprecision(3) << average_duration_us << '\n';
    }
    outfile << '\n';
    outfile.close();
